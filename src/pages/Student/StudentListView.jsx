...
const StudentListIcon = ({ type }) => {
  ...
  return (
    <svg className="sm-list-icon" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
      {paths[type]}
    </svg>
  );
};
...
export default function StudentListView({
  ...
  const modalColumns = useMemo(() => {
    ...
    return [...draftColumns].sort((a, b) => a.order - b.order).filter((column) => column.label.toLowerCase().includes(query));
    ...
    <>
      <div className="student-list-view">
        <AdminListView
          ...
          filterButton={filterButton || <button className="admin-list-filter-btn" type="button" aria-label="Filter" onClick={showFilterMessage}><StudentListIcon type="filter" /></button>}
      ...
      <AdminColumnDrawer
        ...
        searchIcon={<StudentListIcon type="search" />}
        visibleIcon={<StudentListIcon type="eye" />}
        hiddenIcon={<StudentListIcon type="eyeOff" />}
        gripIcon={<StudentListIcon type="grip" />}
      />
    </>
  );
}
